...
public class PhotoPropertiesDiffCopy {
    ...
    /** Initialisation to define the difference. return null if there is no diff between them */
    public PhotoPropertiesDiffCopy setDiff(IPhotoProperties newData, EnumSet<FieldID> diffSet) {
        close();
    ...
    public List<FieldID> applyChanges(IPhotoProperties destination) {
        if (this.numberOfChangedFields > 0) {
            // note: special processing was excluded from this.diffSet

            List<FieldID> collectedChanges = PhotoPropertiesUtil.copySpecificProperties(destination, newData,
